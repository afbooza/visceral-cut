...
- **Auth**: Google OIDC (authorization-code flow) with a legacy shared-secret fallback. `api/auth/login.js` redirects to Google; `api/auth/callback.js` exchanges the code, validates ID-token claims (iss/aud/email_verified) and the email against `ALLOWED_EMAIL`, then mints a 90-day session token (`v1.<base64url payload>.<hmac>`, signed with `SYNC_TOKEN`) handed back via the `/#auth=` URL fragment. The client stores it in localStorage and sends `Authorization: Bearer <token>`; `api/_lib/auth.js` (underscore dir = not a route) verifies either a session token or the raw `SYNC_TOKEN` (legacy). Blob uploads pass the same token via `clientPayload`. Redis credentials stay server-side.
