...
- **Workout programs**: Push/Pull/Legs/Core split, each with warmup exercises and 5-6 main exercises. Sets, reps, and weight are logged per exercise. The stock program lives in the `WORKOUTS` constant; a customized program (exercises swapped in from the catalog via the Plan tab) overrides it per day — always resolve via `getProgram(type)`, never read `WORKOUTS` directly for session/history rendering. History is keyed by exercise *index*, so reordering or removing exercises within a day shifts which historical data shows as PREV — renaming in place is safe. Logs saved since the catalog feature also record a `names` array; the session view suppresses PREV for a slot whose name changed, and history/edit views prefer `log.names` for display.
